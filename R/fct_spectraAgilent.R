...
#' spectrData$info(centroided = TRUE)
#' plot(
#'  spectrData$spectrum(number = 3),
#'  type = "h",
#'  xlab = "m/z", ylab = 'Intensity'
#' )
#' plot(
#'  spectrData$spectrum(number = 4),
#'  type = "h",
#'  xlab = "m/z", ylab = 'Intensity'
#' )
#'
...
mzData <- R6::R6Class(
  ...
  public = list(
    ...
    info = function(number = NA, centroided = FALSE, setSingleRt = 0.05) {
      if ((!(identical(number, NA)) & (length(number) == 1))) {
        ...
        if (!is.na(setSingleRt)){
          for (counter in 1:nrow(result)){
            if (is.na(result$End[counter])){
              result$End[counter] <- result$Start[counter] + setSingleRt
              result$Start[counter] <- result$Start[counter] - setSingleRt
            }
          }
        }
        ...
        return(purrr::map_df(number, ~ self$info(number = .x,
                                                 centroided = centroided,
                                                 setSingleRt = setSingleRt)))
      }
    },
...
#' @title getData.Spectra
#'
#' @description
#'  reads the spectra data file (.mzdata.xml). Uses readFile internally to
#'  prevent stops due to e.g. files not existing
#'
#' @param filename character vector: filename (including path) of the spectra
#'  data
#'
#' @returns an mzData object or NA
#'
#' @examples
#' demoFile <- fs::path_package(
#'   "extdata",
...
#'
#' @export
getData.Spectra <- function(filename = NA){
  if (!purrr::is_empty(filename)){
    if (!is.na(filename)){
      specData <-  readFile(filename = filename, readFunction = mzData$new)(message = FALSE)
      if (!specData$empty){
        return(specData)
      }
    }
